    ...
    for(int row = 7; row >= 0; row--){
        ...
        for(int col = 0; col < 8; col++){
            ...
            if (c >= '1' && c <= '8'){
                ...
                if(piece) board.add_piece(piece, (row * 8) + col);
            }
        }
    }
...
U8 Parser::last_move_sideways(char c){
    if(c == 'b' || c == 'B') return BLACK_MOVED_SIDEWAYS;
    if(c == 'w' || c == 'W') return WHITE_MOVED_SIDEWAYS;
    return 0;
}

U8 Parser::square(char sq[]){
    if((sq[0] < 'A') || (sq[0] > 'G')) return NULL_SQUARE;
    if((sq[1] < '1') || (sq[1] > '7')) return NULL_SQUARE;
...
U32 Parser::move(string str, Board &board){
    ...
    piece = board[from];
    capture = board[to];
    if((from==NULL_SQUARE) || (to==NULL_SQUARE)) return 0;
    if(capture!=EMPTY) {
